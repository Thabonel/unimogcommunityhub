...
Name the document in the first line as: "# PRD — [Feature Title]"
...
Stack: React 18 + TypeScript + Vite + Supabase + Netlify
Key directories:
...
Test commands:
- npm run build (no formal test suite yet)
- npm run lint
- npm run typecheck

Deployment:
- Staging: git push staging main:main (automatic)
- Production: git push origin main (requires permission)

Now generate the PRD/execution prompt based on the user's feature request.
