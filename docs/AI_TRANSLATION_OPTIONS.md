...
async function main() {
  try {
    ...
    if (!process.env.OPENAI_API_KEY) {
      ...
      console.error('Set it in your .env file or run: export OPENAI_API_KEY=<OPENAI_API_KEY>
      process.exit(1);
    }
...
3. **Set API key**: Add `OPENAI_API_KEY=<OPENAI_API_KEY> to your local `.env` file
