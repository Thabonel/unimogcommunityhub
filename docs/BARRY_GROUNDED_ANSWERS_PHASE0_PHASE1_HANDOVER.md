...
## Production Backfill Addendum (2026-08-02)

The production REST path was unavailable (legacy API keys are disabled and no new secret key or database password exists locally), so the Phase 2 steering backfill was delivered as a generated SQL artifact for manual execution:

- `docs/BARRY_PHASE2_PROD_BACKFILL_STEERING_part01_of_08.sql` through `part08_of_08.sql` — 74 documents, 440 evidence units, 1,006 annotations (252 approved, 754 proposed), 199 review items, and the `prod-steering-phase2-1` run record. The single-file version exceeded the SQL editor size limit, so it is split into eight ordered parts (each an independent idempotent transaction). Run part01 through part08 in order. Verified end-to-end on a fresh local database: all eight parts apply in sequence, produce the exact expected counts, and tolerate re-execution.
- Generated by `scripts/barry-backfill/generate-backfill-sql.ts` from a local PostgreSQL run over a read-only production extract (steering scope: workshop manual pages 600-615 and 920-960, RPS Catalog pages 590-630, the three steering chapter manuals, RPS groups PA/PB/PBA/PBB).
- Verified standalone on a fresh local database containing only the Phase 1 and Phase 2 migrations: first apply produced the exact expected counts; second apply was idempotent; `rollback_barry_backfill_run('prod-steering-phase2-1')` removed exactly the 440 units and 1,006 annotations.
- Concept and version references resolve by subquery (`concept_key`, `version`), so the script binds to production IDs at execution time.
