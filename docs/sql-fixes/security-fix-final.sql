-- Security Fix: Final Version (UUID Fixed)
-- Date: 2026-02-10
-- Purpose: Fix security issues with proper UUID handling
...
BEGIN
  ...
  IF EXISTS (SELECT 1 FROM pg_tables WHERE schemaname = 'public' AND tablename = 'spatial_ref_sys') THEN
    ...
    EXCEPTION WHEN OTHERS THEN
      RAISE NOTICE 'Could not modify spatial_ref_sys (system table): %', SQLERRM;
    END;
  END IF;
END $$;
...
BEGIN
  IF EXISTS (SELECT 1 FROM pg_views WHERE schemaname = 'public' AND viewname = 'barry_knowledge_analytics') THEN
    ...
    EXCEPTION WHEN OTHERS THEN
      -- Fixed: Use NULL for UUID, not empty string
      CREATE VIEW public.barry_knowledge_analytics
      WITH (security_invoker = true) AS
      SELECT
        NULL::uuid as id,
        NULL::text as question_hash,
        NULL::text as answer,
...
BEGIN
  IF EXISTS (SELECT 1 FROM pg_views WHERE schemaname = 'public' AND viewname = 'vw_recent_wis_properties') THEN
    ...
    EXCEPTION WHEN OTHERS THEN
      -- Fixed: Use NULL for UUID
      CREATE VIEW public.vw_recent_wis_properties
      WITH (security_invoker = true) AS
      SELECT
        NULL::uuid as id,
        NULL::text as property_name,
        NULL::text as property_value,
...
BEGIN
  IF EXISTS (SELECT 1 FROM pg_views WHERE schemaname = 'public' AND viewname = 'vw_pending_synonyms') THEN
    ...
    EXCEPTION WHEN OTHERS THEN
      ...
      SELECT
        NULL::uuid as id,
        NULL::text as original_term,
        NULL::text as suggested_synonym,
        0.0 as confidence_score,
        NOW() as created_at,
        NULL::text as status
      WHERE false;
...
SELECT
  ...
  'All UUID casting issues fixed' as note,
  'Re-run Supabase Database Linter to verify 0 issues' as next_step;