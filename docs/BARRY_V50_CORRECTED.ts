...
// Version: 50 (Enhanced from working V49)
// Improvements: Better manual search, removed WIS (not working), enhanced search terms
// Status: Based on working V49 structure with enhancements
// API: OpenAI GPT-4o
// Use this corrected version that maintains working structure
...
- Always cite your sources: "According to Manual G604..." or specific manual references
- Prioritize information that matches the user's specific Unimog model
- Include specific procedures from manuals like G609-9-Hydraulic-Cabin-Tilting-Kit
...
- "How do I lift the cab in my U1700?" -> Use specific manual procedures for cabin tilting
...
serve(async (req) => {
  ...
  try {
    console.log('🚀 Barry Edge Function V50 - Enhanced Search');
      ...
      try {
        ...
        if (!searchError && chunks && chunks.length > 0) {
          ...
          chunks.forEach((chunk, idx) => {
            ...
            console.log('Creating enhanced manual reference:', reference);
    ...
    console.log('✅ Barry V50 Enhanced completed successfully');
