...
async function generateSQL() {
  console.log('Reading extracted data from pages 201-221...');

  const extractedPath = path.join(__dirname, 'PAGES_201_221_EXTRACTED.json');
...
-- RPS Parts List - Pages 201-221 Extracted Data (133 new items)
-- Generated from PAGES_201_221_EXTRACTED.json
...
`;
  ...
  const outputPath = path.join(__dirname, '..', '..', 'docs', 'rps-pages-201-221-insert.sql');
  ...
  console.log(`\nThis SQL inserts NEW items from pages 201-221 that were not in previous extractions.`);
  console.log(`Combined with all previous batches: 382 + 79 + 143 + 89 + ${totalItems} = ${382 + 79 + 143 + 89 + totalItems} total items`);
}
