...
async function generateSQL() {
  console.log('Reading extracted data from pages 427-627...');

  const extractedPath = path.join(__dirname, 'PAGES_427_627_EXTRACTED.json');
...
-- RPS Parts List - Pages 427-627 Extracted Data (99 new items)
-- Generated from PAGES_427_627_EXTRACTED.json
...
`;
  ...
  const outputPath = path.join(__dirname, '..', '..', 'docs', 'rps-pages-427-627-insert.sql');
  ...
  console.log(`\nThis SQL inserts NEW items from pages 427-627 that were not in previous extractions.`);
  console.log(`Combined with all previous batches: 382 + 79 + 143 + ${totalItems} = ${382 + 79 + 143 + totalItems} total items`);
}
