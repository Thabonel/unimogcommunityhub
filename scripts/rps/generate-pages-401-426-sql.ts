...
async function generateSQL() {
  console.log('Reading extracted data from pages 401-426...');

  const extractedPath = path.join(__dirname, 'PAGES_401_426_EXTRACTED.json');
...
-- RPS Parts List - Pages 401-426 Extracted Data (164 new items)
-- Generated from PAGES_401_426_EXTRACTED.json
...
`;
  ...
  const outputPath = path.join(__dirname, '..', '..', 'docs', 'rps-pages-401-426-insert.sql');
  ...
  console.log(`\nThis SQL inserts NEW items from pages 401-426 that were not in previous extractions.`);
