...
/**
 * Resilient RPS OCR Batch Processor - WITH LOCAL IMAGE DOWNLOAD
 *
 * Improvements:
 * - Downloads images locally before sending to OpenAI
 * - Converts to base64 to avoid OpenAI timeout on URL downloads
...
// OCR a single page with retry logic - USING LOCAL IMAGE
...
Format the output as clean, structured text that preserves the table data. Be thorough and accurate.`;
  ...
  try {
    ...
    log(`  📥 Downloading image from Supabase...`);
    const imageBuffer = await downloadImage(page.page_image_url);
    const base64Image = imageBuffer.toString('base64');
    log(`  ✓ Downloaded ${(imageBuffer.length / 1024).toFixed(1)}KB, converted to base64`);
    ...
    log(`  🤖 Sending to OpenAI GPT-4o...`);
    const response = await makeRequest(options, requestBody, 120000); // 120 second timeout
...
async function main() {
  log('==========================================');
  log('RPS OCR Processing - Local Image Download');
  log('==========================================');
  log('');
  ...
  log(`Estimated time: ~${Math.ceil(pages.length * 10 / 60)} minutes`);
  log('');
