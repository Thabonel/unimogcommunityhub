...
const FiresMapV2 = ({ 
  ...
  // Initialize map with proper load event handling
  useEffect(() => {
    if (!mapContainerRef.current || mapRef.current) return;
    
    const token = getMapboxTokenFromAnySource();
    if (!token) {
      console.error('FiresMapV2: No Mapbox token available');
      setIsLoading(false);
      return;
    }
    ...
    try {
      // Set token right before map creation
      mapboxgl.accessToken = token;
      
      // Create map instance with exact pattern as working maps
      ...
      // Critical: Handle load event properly
      map.on('load', () => {
        console.log('FiresMapV2: Map loaded successfully');
      ...
      // Handle map errors
      map.on('error', (e) => {
        console.error('FiresMapV2: Map error:', e);
      ...
      console.error('FiresMapV2: Failed to initialize map:', error);
      setIsLoading(false);
    }
  ...
  }, [isMapReady, radius, userLocation, location]);
    ...
    <div className="space-y-4">
      ...
      <div className="relative">
        <div 
          ...
          style={{ minHeight: '400px' }} // Critical: ensures container has height!
        />
...
export default memo(FiresMapV2);