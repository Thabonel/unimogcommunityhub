...
import { hasMapboxToken } from '../utils/tokenUtils';
import { loadTripsOnMap } from '../utils/mapMarkerUtils';
import { updateMapView } from '../utils/mapNavigationUtils';
...
export const useMapLocations = (props?: UseMapLocationsProps) => {
  ...
  useEffect(() => {
    if (props && props.map && !props.isLoading && !props.error) {
      ...
      // Handle route display via the useMapRouteDisplay hook
      if (props.startLocation || props.endLocation) {
        ...
        // The actual route display is now handled by useRouteDisplay
      }
    }
  ...
  useEffect(() => {
    if (mapRef.current && trips && trips.length > 0) {
      loadTripsOnMap(mapRef.current, trips);
    }
  }, [trips]);
  ...
  const handleTripSelection = (trip: Trip) => {
    ...
    if (trip.start_location?.latitude && trip.start_location?.longitude) {
      updateMapView(mapRef.current, trip.start_location, 10);
    }
  };
  ...
  const clearSelectedTrip = () => {
    ...
    if (mapRef.current) {
      updateMapView(mapRef.current, { latitude: 40.0, longitude: -99.5 }, defaultZoom);
    }
  };
