...
import { validateFile, ALLOWED_FILE_TYPES, FILE_SIZE_LIMITS } from "@/utils/fileValidation";
...
export function FileDropArea({ onFileSelected }: FileDropAreaProps) {
  ...
  const validateFileUpload = (file: File): boolean => {
    const { valid } = validateFile(file, {
      ...
      maxSize: FILE_SIZE_LIMITS.manual
    });
    return valid;
  };
  ...
  return (
    ...
    >
      ...
      <p className="text-xs text-muted-foreground mt-2">
        PDF, TXT, DOC, DOCX, RTF, MD files supported (max 200MB)
      </p>
      {isDragging && (
