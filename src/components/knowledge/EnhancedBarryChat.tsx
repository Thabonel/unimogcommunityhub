...
export function EnhancedBarryChat({ className, location, userModel }: EnhancedBarryChatProps) {
  ...
  const checkForDiagramRequest = (message: string): string | null => {
    ...
    // Direct keyword matching - more flexible detection
    if (lowerMessage.includes('portal') || lowerMessage.includes('drain plug') || lowerMessage.includes('fill plug')) {
      return 'portal_axle';
    }
    if (lowerMessage.includes('differential') || lowerMessage.includes('diff lock')) {
      return 'differential';
    }
    if (lowerMessage.includes('oil') && (lowerMessage.includes('flow') || lowerMessage.includes('circuit'))) {
      return 'oil_circuit';
    }
    if (lowerMessage.includes('wiring') || lowerMessage.includes('electrical')) {
      return 'wiring';
    }

    // Explicit diagram requests
    if (lowerMessage.includes('show') || lowerMessage.includes('diagram') || lowerMessage.includes('illustration')) {
      if (lowerMessage.includes('portal') || lowerMessage.includes('axle')) return 'portal_axle';
      if (lowerMessage.includes('differential')) return 'differential';
      if (lowerMessage.includes('oil')) return 'oil_circuit';
      if (lowerMessage.includes('electrical')) return 'wiring';
    }
