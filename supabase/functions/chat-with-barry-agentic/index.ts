...
function prioritizeSearchResults(searchResults: any[], userQuery: string): any[] {
  ...
  // Smart sorting: Prioritize by keyword match + relevance
  const sortedResults = searchResults.sort((a, b) => {
    // Calculate keyword match score (how many query words appear in the term)
    const aTermLower = (a.term || '').toLowerCase();
    const bTermLower = (b.term || '').toLowerCase();
    const aKeywordMatches = queryWords.filter(word => aTermLower.includes(word)).length;
    const bKeywordMatches = queryWords.filter(word => bTermLower.includes(word)).length;

    // Prioritize results that match more query keywords
