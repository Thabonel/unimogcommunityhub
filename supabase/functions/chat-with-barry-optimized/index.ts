...
- VIN: ${vehicleData.vin || 'Not provided'}`
    ...
    if (lastUserMessage && lastUserMessage.content) {
      ...
      if (isVehicleQuery && userVehicleProfile) {
...
INTERNET RESEARCH CONTEXT for ${userVehicleProfile.year} ${userVehicleProfile.userModel}:
...
- Your ${userVehicleProfile.wisSeries} series has specific procedures that differ from other Unimog models
- Engine type ${userVehicleProfile.engineCode} has particular requirements and specifications
- Technical documentation for your model year ${userVehicleProfile.year} includes updated procedures
- Compatible parts and fluids are specified for your ${userVehicleProfile.unimogSeries} series
...
This research was conducted specifically for your vehicle configuration, not generic Unimog information.`
    ...
    if (useOptimized && userVehicleProfile && lastUserMessage) {
    ...
    console.error('Error in Barry optimized function:', error)
