...
-- Issue 3: Enable RLS on spatial_ref_sys table
-- This is a PostGIS system table - should be readable by everyone, writable by admins only
ALTER TABLE public.spatial_ref_sys ENABLE ROW LEVEL SECURITY;

-- Policy: Everyone can read spatial reference systems (needed for mapping)
...
-- Policy: Only admins can modify spatial reference systems
CREATE POLICY "Admins can modify spatial reference systems"
  ON public.spatial_ref_sys
  FOR ALL
  TO authenticated
  USING (check_admin_access())
